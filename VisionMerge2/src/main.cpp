...
void loop()
{
  ...
  */
  ...
  while (i == 0)
  {
    ...
    connected = true;
    driveUnity.forward(speed - 48, motor_prop, kp, ki, kd);
  ...
  ind_semicolon = msg.indexOf(";");
  str_param = msg.substring(0, ind_semicolon); // get the
  motor_prop = str_param.toFloat();            // 3
  Serial.println(motor_prop, DEC);
  msg = msg.substring(ind_semicolon + 1, msg.length());
  ...
  ind_semicolon = msg.indexOf(";");
  str_param = msg.substring(0, ind_semicolon); // get the
  kp = str_param.toFloat();                    // this is the first float
  ...
  ind_semicolon = msg.indexOf(";");
  str_param = msg.substring(0, ind_semicolon); // get the
  ki = str_param.toFloat();                    // this is the second float
  ...
  ind_semicolon = msg.indexOf(";");
  str_param = msg.substring(0, ind_semicolon); // get the
  kd = str_param.toFloat();                    // this is the first float
  Serial.println("extract float");
  Serial.println(kd, 3); // print to 3 degrees of precision (3 d.p.)
