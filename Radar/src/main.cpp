...
// for(int i = 0; i<data[o-1]-1;i++){
// if(out_im[i] != 0){
//   out_im_final[i] = out_im[i];
// }
// }

    for(int i=0;i<5;i++)     // updating f_peak array (global variable)with descending order
    {
    //f_peaks[i]=out_im_final[in_ps[i]];
   ...
   // int arr[] = { 1, 2, 3, 4 };
    ...
    Serial.println("Array of ascending frequencies:");
...
For Arduino nano, FFT of more than 128 sample not possible due to memory limitation (64 recomended)
...
for(int i = 0; i < 128; i++)
{
data_in[i] = analogRead(A4); // Set you analog pin, which is ADC1_CH4 according to ESP32
