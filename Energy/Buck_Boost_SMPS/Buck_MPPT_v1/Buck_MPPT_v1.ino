...
void loop() {
    ...
    // Measure output current from port B in mA
    ...
    // Measure output voltage from port B 
    sensorValue0 = analogRead(A0);
    vout = sensorValue0 * 4.096/1.03 *3.86; // using 560k and 200k resistor as potential divider

    //for debugging - print output voltage
    Serial.print("vout (mA): ");
    Serial.println(vout);

    // Measure the input voltage from port A
    sensorValue1 = analogRead(A3);
    vpd = sensorValue1 * (4.096/1.03) * 2.05 ; // I am using 2 470k resistor as the potential divider
    vin_present = vpd * 2.70;
    ...
    power_present = vout*current_present;
