...
void loop() {
  ...
  if(loop_trigger == 1){ // FAST LOOP (1kHz)
    ...
    if (vout_present < 5000){ // To present output voltage above 5.00V and damage the battery :(
      if (v_diff>0){
    ...
    pwm_out = saturation(pwm_out,1,0); // Saturate the duty cycle at the reference of a min of 0.02
