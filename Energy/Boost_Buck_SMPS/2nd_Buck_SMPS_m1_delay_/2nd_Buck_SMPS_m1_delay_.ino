...
void loop() {
  ...
  if (vout_present<5200){
    digitalWrite(5,HIGH);
  }else if (vout_present>4500 && vout_present <5000){
    digitalWrite(5,LOW);
  }else{
    digitalWrite(5,HIGH);
  }
  ...
  if(loop_trigger == 1){ // FAST LOOP (1kHz)
    ...
    if (vout_present < 5000){ // To prevent output voltage above 5.00V and damage the battery :(
      if (v_diff>0){
...
//      Serial.println(power_present/1000);
   ...
   dataString = String("Final Voltage Output (V)") + "," + String(vout_present/1000) + "," + String("Input Voltage from 2nd SMPS (V)") + "," + String(vin/1000) + "," + String("Vref(V) ") + "," + String(vref/1000) + "," + String("Output Current (A)") + "," + String(current_present/1000) + "," + String("DutyCycle of 2nd SMPS") + "," + String(pwm_out) + "," + String("Output Power(mW) ") + "," + String(power_present/1000);//build a datastring for the CSV file
